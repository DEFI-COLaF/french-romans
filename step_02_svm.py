# -----------------------------------------------------------------------------
# step_02_svm.py – corrected version
# -----------------------------------------------------------------------------
"""Benchmark BDI / SVM pour l'attribution d'auteurs.
"""
...
def _train_test_split_imp(df: pd.DataFrame, test_frac: float = TEST_FRAC_IMP):
    """
    Sépare df en train/test en s'assurant qu'il y ait au moins un imposteur
    dans le jeu d'entraînement et (si possible) un dans le test.
    """
    if df.empty:
        return df, df

    if len(df) == 1:
        return df.iloc[:0], df        # un seul imposteur → tout en test

    test_df = df.sample(frac=test_frac, replace=False, random_state=rng)
    if test_df.empty:                 # si le tirage donne 0 ligne
...
def run_bdi(pickled_experiment: bytes) -> Dict[str, Any]:
    ...
    X_train = pd.concat([experiment.candidate[experiment.features],
                         imp_train[experiment.features]])
    y_raw = pd.concat([experiment.candidate["var_author"],
                       imp_train["var_author"]])
    y, _ = y_raw.factorize()

    scaler = StandardScaler(with_mean=False).fit(X_train)
    verifier = BDIVerifier(metric="minmax",
    ...
    labels = [f"{fn}#{w}" for w, fn in zip(experiment.query["var_window"],
                                           experiment.query.index)]
...
def run_svm(pickled_payload: bytes,
            fast: bool = False,
            search_mode: str = "halving",
            n_ensemble: int = 20) -> Optional[Dict[str, Any]]:
    ...
    for rep in range(n_ensemble):
        ...
        X_train = pd.concat([cand_boot[experiment.features],
                             imp_boot[experiment.features]])
        y_raw   = pd.concat([cand_boot["var_author"],
                             imp_boot["var_author"]])
        y = (y_raw == experiment.author).astype(int).values

        # -------- pare-feu contre potentielle classe unique ----------------------------------
        ...
        if fast:
            ...
            search = SearchCls(Pipeline([("svc", svc)]),
                               {"svc__C": C_GRID},
                               scoring="roc_auc",
...
if __name__ == "__main__":
    ...
    metadata_cols      = [c for c in authors.columns if c not in authors_features]
    ...
    for gap, ascending in product([1, 5, 10, 15], [True, False]):
        ...
        with ProcessPoolExecutor(max_workers=NB_PROCS) as ex:
            ...
            for fut in as_completed(futs):
                res = fut.result()
                if res is not None:          # filtre les None
                    results.append(res)
                bar.update(1)
        ...
        fname = (
            f"results-{suffix}{'-fast' if args.fast and suffix=='svm' else ''}"
            f"-ens{args.ensemble}-{gap}-{ascending}.json"
        )
